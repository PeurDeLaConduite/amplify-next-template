...
export function crudService<K extends ClientModelKey>(
    ...
    return {
        async list() {
            const { data } = await model.list({ authMode: authValue });
            return {
        ...
        create: model.create({ authMode: authValue }),
        update: model.update({ authMode: authValue }),
        delete: model.delete({ authMode: authValue }),
    };
}
