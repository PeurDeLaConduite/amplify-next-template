...
>(options: UseModelFormOptions<F, E>): UseModelFormResult<F, E> {
  const { initialForm, initialExtras, mode: initialMode = "create", validate, create, update, syncRelations } = options;
  ...
  const submit = useCallback(async () => {
    ...
    try {
      ...
      setMode("edit");
      initialRef.current = form; // baseline = form sauvegardé
    } catch (e) {
      setError(e);
    } finally {
      setSaving(false);
  ...
  return {
    form, extras, mode, dirty, saving, error, message,
    handleChange, submit, reset, setForm, setExtras, setMode, setMessage,
    adoptInitial, // ← expose le helper
  };
}
