...
export function renderModelForm(m: ModelMeta, relations: RelationsMap) {
  ...
  const simple = m.fields.filter((f) => ["string","number","boolean","id","enum"].includes((f as any).kind));
  ...
  const ctImports = custom.map((cf) => `import { initial${cf.refType}Form, to${cf.refType}Form } from "${GEN.paths.customTypeFormDir(cf.refType)}";`).join("\n");

  const initLines: string[] = [];
  if (GEN.rules.includeIdInForm) initLines.push(`  id: "",`);
  for (const f of simple) {
    const name = (f as any).name; const kind = (f as any).kind;
  ...
  for (const f of simple) {
    const name = (f as any).name; const kind = (f as any).kind;
    if (name === "id") continue;
    if (kind === "string" || kind === "id" || kind === "enum") toFormLines.push(`  ${name}: model.${name} ?? "",`);
    else if (kind === "number") toFormLines.push(`  ${name}: model.${name} ?? 0,`);
    else if (kind === "boolean") toFormLines.push(`  ${name}: model.${name} ?? false,`);
  }
  for (const cf of custom) toFormLines.push(`  ${cf.name}: to${cf.refType}Form(model.${cf.name}),`);
  for (const k of relKeysSingular) toFormLines.push(`  ${k}Ids,`);

  const argsTupleType = manyToMany.length ? `[${manyToMany.map(() => "string[]").join(", ")}]` : "[]";
  const argsDecl = manyToMany.length ? ", " + relKeysSingular.map((k) => `${k}Ids: string[] = []`).join(", ") : "";
  const argsCall = manyToMany.length ? ", " + relKeysSingular.map((k) => `${k}Ids`).join(", ") : "";
...
export const ${low}Form = createModelForm<${m.name}Type, ${m.name}FormType, ${argsTupleType}>(
  initial${m.name}Form,
