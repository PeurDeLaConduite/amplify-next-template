...
async function main() {
  ...
  for (const m of metas) {
    ...
    const hasComposite = Array.isArray((m as any).identifier) && (m as any).identifier.length > 0;
    const hasId = m.fields.some((f) => (f as any).name === "id" && (f as any).kind === "id");
    if (!hasComposite && !hasId) (m as any).fields.unshift({ name: "id", kind: "id", required: true });
  }
  ...
  for (const m of metas.filter((m) => m.type === "model" && (pivots as string[]).includes(m.name))) renderPivot(m as any);
  for (const m of metas.filter((m) => m.type === "model" && !(pivots as string[]).includes(m.name))) renderModel(m as any, relations as any);
